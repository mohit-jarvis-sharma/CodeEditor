...
def index_editor(request,id):
    user = Users.objects.get(user=request.user)
    student_data = Student.objects.get(User=user)
    section = StudentSectionRelation.objects.get(Student__User=user)
    question = get_object_or_404(Problem, pk = id)
    return render(request, 'assignment_dashboard/main_editor.html', {'question': question,'student_data': student_data, 'section' : section})